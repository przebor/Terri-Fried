...
int main(void)
{
    ...
    ret = debugNetInit(DEBUG_IP,DEBUG_PORT,DEBUG);
    ...
    while (true)
    {
        ...
        if (titleScreen)
        {
            if (splashTimer > 120)
            {
                ...
                vita2d_clear_screen();
                vita2d_draw_texture(logo, SCREEN_WIDTH/2 - 200, SCREEN_HEIGHT/2 - 45 - 30);
                vita2d_font_draw_text(font, SCREEN_WIDTH/2 - 41, SCREEN_HEIGHT/2 + 10 + 32, RGBA8(0, 0, 0, 255), 32, highscore);
                vita2d_font_draw_text(font, SCREEN_WIDTH/2 - 146, SCREEN_HEIGHT/2 + 50 + 32, RGBA8(178, 150, 125, 102), 32, "CLICK ANYWHERE TO BEGIN");
                vita2d_end_drawing();
                vita2d_swap_buffers();
                if (click())
                {
                ...
                vita2d_start_drawing();
                vita2d_clear_screen();
                vita2d_font_draw_text(font, SCREEN_WIDTH/2 - 59, SCREEN_HEIGHT/2 + 3 + 26, RGBA8(213, 128, 90, 255), 32, "POLYMARS");
                vita2d_draw_texture(splashEggSprite, SCREEN_WIDTH/2 - 16, SCREEN_HEIGHT/2 - 16 - 23);
                vita2d_end_drawing();
                vita2d_swap_buffers();
                splashTimer += 1;
            }
